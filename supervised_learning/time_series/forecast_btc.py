...
def implement_dataset(df, sequence_length):
    """ BTC WIP """
    data = df.values
    targets = data[sequence_length:]  # targets are the values right after each sequence
    data = data[:-sequence_length]  # adjust data to match the size of targets
...
test_dataset = implement_dataset(test_df, sequence_length)  # You might need targets for testing as well
