...
def cumulative_bleu(references, sentence, n):
    ...
    closest_ref_length = min(ref_lengths, key=lambda ref_len: (abs(ref_len - sen_length), ref_len))
    brevity_penalty = np.exp(1 - closest_ref_length / sen_length) if sen_length < closest_ref_length else 1
    ...
    for i in range(1, n+1):
        ...
        clipped_counts = {ngram: min(count, max_counts[ngram]) for ngram, count in sentence_ngrams.items()}
        precision = sum(clipped_counts.values()) / sum(sentence_ngrams.values())
